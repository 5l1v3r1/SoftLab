...
public class Parser {
    ...
    private Scanner scanner; //Scanner returning token to parse
    private Token token; //Current token
    private Token nextToken; //Look ahead token
    public int errors; //Errors count
    private SymbolTable table; //Symbol table
    private SymObject currentMethod;  // Current method. This reference is retained for some semantic checks

    public Code code = new Code(); //Generated code buffer
    ...
    private void error(String message) { // syntactic error at token la
        errors++;
