...
public class Scanner {
    public static final char CHAR_EOF = '\u0080'; // character that is returned at the end of the file
    private static final char CHAR_EOL = '\n';

    public static final Map<String, Integer> KEYWORDS = buildKeywordsMap();  //List of keywords and associated token code

    private InputStream in; // source file reader

    private char ch;		// lookahead character
     ...
     */
    public Token next() {
        // Skip non-printing characters like tab,blanks etc.
        while (ch <= ' ') {
            nextCh();
        }
        ...
        if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z')) {
            ...
            // read keywords,special characters
        } else {
            switch (ch) {
                case '\'':
    ...
    /**
     * Internal method, builds the map associating keywords literals to their corresponding token codes.
     * @return
     */
